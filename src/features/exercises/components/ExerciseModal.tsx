...
export function ExerciseModal({ isOpen, onClose, mode, initialData, onSuccess }: ExerciseModalProps) {
  ...
  // Fetch all exercises to populate selectors
  const { data: exercises = [] } = useFirestoreQuery<Exercise>(
    ['exercises_for_selectors'],
    'exercises',
    [], 
    1000 * 60 * 60 
  );
  ...
  // Categorías
  const uniqueGroups = Array.from(new Set(exercises.map(ex => ex.muscle_group).filter(Boolean)));
  ...
  // Equipamientos
  const uniqueEquipments = Array.from(new Set(exercises.map(ex => ex.equipment).filter(Boolean)));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'create') {
        ...
        await updateDoc(doc(db, 'exercises', initialData.id), {
          name,
          muscle_group: muscleGroup,
          equipment
        });
